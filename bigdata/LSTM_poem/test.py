from bigdata.LSTM_poem.generate import *
from bigdata.LSTM_poem.model import PoetryModel
...
def init():
    ...
    model = PoetryModel(len(idx2word), Config.embedding_dim, Config.hidden_dim)
...
def generateApi(start_words):
    gen_poetry = ''.join(generate(model, start_words, idx2word, word2idx))
    return gen_poetry


def gen_acrosticApi(start_words):
    gen_poetry = ''.join(gen_acrostic(model, start_words, idx2word, word2idx))
    return gen_poetry
